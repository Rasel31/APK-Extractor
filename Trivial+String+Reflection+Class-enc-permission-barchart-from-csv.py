...
file_path = "D:\\Androguard\\androguard-master\\" \
            "TRIVIAL+STRING_ENCRYPTION+REFLECTION+CLASS_ENC-permission-pattern.csv "
...
plt.xticks(index[:50], sorted(a.items(), key=itemgetter(1), reverse=True)[50:100], fontsize=6, rotation=90)
# plt.xticks(index[:53], sorted(a.items(), key=itemgetter(1), reverse=True)[100:], fontsize=10, rotation=90)

# plt.title('0 to 50 of 107 TRIVIAL+STRING+REFLECTION+CLASS ENC. Permissions')
plt.title('51 to 100 of 107 TRIVIAL+STRING+REFLECTION+CLASS ENC. Permissions')
# plt.title('101 to Last of 107 TRIVIAL+STRING+REFLECTION+CLASS ENC. Permissions')
...
# with open('trivial+string+reflection+class-enc-permission.txt', 'w', encoding='utf-8') as f:
#     count = 1
